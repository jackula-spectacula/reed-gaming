...
#' Generate compact HTML for Cubs/Braves box scores only (no umpires, opponent pitching optional)
#'
#' @param games_data List of games' data
#' @param include_opponent_pitching Logical, whether to include opponent pitching stats
#' @return HTML string for the box scores section
generate_boxscores_html <- function(games_data, include_opponent_pitching = TRUE) {
  # Filter to only Cubs and Braves games
  target_teams <- c("CHC", "ATL")
  filtered_games <- Filter(function(game) {
    game$teams$visitor$abbrev %in% target_teams || game$teams$home$abbrev %in% target_teams
  }, games_data)

  # Sort: Cubs first, Braves second
  game_priority <- sapply(filtered_games, function(game) {
    teams <- c(game$teams$visitor$abbrev, game$teams$home$abbrev)
    if ("CHC" %in% teams) return(1)
    if ("ATL" %in% teams) return(2)
    return(3)
  })
  filtered_games <- filtered_games[order(game_priority)]

  if (length(filtered_games) == 0) {
    return("    <div class='boxscores-container'><div class='notes'>No Cubs or Braves games yesterday.</div></div>\n")
  }
  ...
  for (i in seq_along(filtered_games)) {
    ...
    # Determine which team is Cubs/Braves (the "target" team)
    is_visitor_target <- game$teams$visitor$abbrev %in% target_teams
    is_home_target <- game$teams$home$abbrev %in% target_teams

    # Pitching tables - Visitor (skip if opponent and include_opponent_pitching is FALSE)
    if (include_opponent_pitching || is_visitor_target) {
      html <- paste0(
        html,
    ...
    # Pitching tables - Home (skip if opponent and include_opponent_pitching is FALSE)
    if (include_opponent_pitching || is_home_target) {
      html <- paste0(
        html,
        "          <table class='pitching-table'>\n",
        "          <thead>\n",
    ...
    # Game info (time, attendance only - NO umpires for compact version)
    c_gameInfoParts <- c()
...
generate_newspaper_page2 <- function(games_data, date_str,
  ...
  # Compact 8x11 version - standings + Cubs/Braves box scores only
  html_content <- paste0(
    "<!DOCTYPE html>\n",
    "<html>\n",
    "<head>\n",
    ...
    "  <title>MLB Daily - ", display_date, "</title>\n",
    "  <style>\n",
    ...
    "    body { font-family: 'Source Sans 3', 'Segoe UI', sans-serif; font-size: 10px; line-height: 1.15; color: var(--text-primary); background: var(--bg-page); }\n",
    "    .newspaper { max-width: 8in; margin: 0 auto; padding: 0.25in; }\n",
    "    .header { text-align: center; border-bottom: 2px solid var(--border-strong); padding-bottom: 4px; margin-bottom: 6px; }\n",
    "    .date { font-size: 10px; font-style: italic; }\n",
    "    .main-title { font-size: 22px; font-weight: bold; margin: 2px 0; }\n",
    "    .section { margin-bottom: 6px; }\n",
    "    .column-container { display: flex; gap: 10px; }\n",
    "    .column-half { flex: 1; }\n",
    "    .boxscores-title { text-align: center; font-size: 12px; font-weight: bold; margin: 4px 0 2px 0; border-bottom: 1px solid var(--border-strong); }\n",
    "    .stats-subheader { font-size: 10px; font-weight: bold; margin: 3px 0 1px 0; border-bottom: 1px solid var(--border-light); }\n",
    "    .standings-table { width: 100%; border-collapse: collapse; font-size: 9px; table-layout: fixed; }\n",
    "    .standings-table th { border: none; font-weight: bold; font-size: 8px; }\n",
    "    .standings-table td { line-height: 1.2; }\n",
    "    .standings-table .team-col { width: 22%; text-align: left; }\n",
    "    .standings-table .num-col { width: 6%; text-align: right; }\n",
    "    .standings-table .pct-col { width: 9%; text-align: right; }\n",
    "    th:not(.team-col), td:not(.team-col) { text-align: right; }\n",
    "    td { white-space: nowrap; }\n",
    "    .leaders-note { font-size: 8px; margin: 2px 0; }\n",
    "    .boxscores-container { column-count: 2; column-gap: 12px; margin-top: 4px; }\n",
    "    .game-container { break-inside: avoid; page-break-inside: avoid; margin-bottom: 8px; }\n",
    "    .game-header { font-weight: bold; font-size: 12px; border-bottom: 1px solid var(--border-strong); }\n",
    "    .team-line { display: flex; justify-content: space-between; font-size: 10px; font-weight: 700; line-height: 1.2; margin-top: 1px; }\n",
    "    table { width: 100%; border-collapse: collapse; font-size: 9px; table-layout: fixed; }\n",
    "    .batting-table tr:last-child { font-weight: 700; }\n",
    "    th, td { text-align: left; overflow: hidden; line-height: 1.15; }\n",
    ...
    "    .notes { font-size: 9px; line-height: 1.15; padding: 2px 0; border-top: 1px solid var(--border-strong); }\n",
    "    @media print {\n",
    "      @page { size: 8.5in 11in; margin: 0.25in; }\n",
    "      body { font-size: 9px; }\n",
    "      .newspaper { max-width: none; padding: 0; margin: 0; }\n",
    "    }\n",
    "  </style>\n",
    "</head>\n",
    "<body>\n",
    ...
    "      <h1 class='main-title'>MLB DAILY</h1>\n",
    "    </div>\n"
  )

  # Add NL standings first, then AL standings (no leaders)
  if (!is.null(standings_data)) {
    ...
    html_content <- paste0(
      ...
      ifelse(length(nl_clinch_footnotes) > 0,
             paste0("          <div class='leaders-note' style='margin-top: 4px;'>", paste(nl_clinch_footnotes, collapse = "; "), "</div>\n"),
             ""),
      "        </div>\n",
      "        <div class='column-half'>\n",
    ...
    html_content <- paste0(
      ...
      ifelse(length(al_clinch_footnotes) > 0,
             paste0("          <div class='leaders-note' style='margin-top: 4px;'>", paste(al_clinch_footnotes, collapse = "; "), "</div>\n"),
             ""),
      "        </div>\n",
      "      </div>\n",
      "    </div>\n"
  ...
  # Add Cubs/Braves box scores only (no umpires, include opponent pitching by default)
  html_content <- paste0(html_content, generate_boxscores_html(games_data, include_opponent_pitching = FALSE))
...
# Original generate_newspaper_page2 code removed - replaced with compact version above

if (FALSE) {
  DEAD_CODE_START <- TRUE
...
get_box_scores <- function(year, month, day,
                           season = NULL,
                           output_dir = "../shortball",
                           save_newspaper = TRUE,
                           save_pdf = TRUE,
                           include_standings = TRUE,
                           include_leaders = FALSE,
                           include_games_schedule = FALSE) {
  ...
  # Generate and save compact page
  if (save_newspaper) {
    print("Generating compact page...")
    ...
    # No leaders or games schedule for compact version
    leaders_data <- NULL
    games_schedule_data <- NULL

    # Generate the compact page
    print("Creating compact HTML...")
    ...
    newspaper_file <- file.path(output_dir, paste0(date_str, "_compact.html"))
    print(paste0("Writing compact page to ", newspaper_file))
    writeLines(newspaper_html, newspaper_file)

    # Also write as index.html so jackulaspectacula.com/shortball always shows latest
    index_file <- file.path(output_dir, "index.html")
    writeLines(newspaper_html, index_file)
    print("Compact HTML file created successfully")
  }

  # Generate PDF if requested (portrait 8.5x11)
  if (save_newspaper && save_pdf) {
    print("Generating PDF version...")
    pdf_file <- file.path(output_dir, paste0(date_str, "_compact.pdf"))
    tryCatch({
      print_to_pdf(paste0('file://', here(newspaper_file)), here(pdf_file), landscape=FALSE, paperWidth=8.5, paperHeight=11, wait_ = TRUE)
